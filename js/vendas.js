...
function adicionarItemEdicaoOrcamento() {
    itensEdicaoOrcamento.push({ id: '', produtoId: '', nome: '', qtd: 1, unitario: 0, total: 0, origem: 'ORCAMENTO_MANUAL' });
    renderizarItensEdicaoOrcamento();
}

function atualizarItemEdicaoOrcamento(indice, campo, valor) {
    const item = itensEdicaoOrcamento[indice];
    if (!item) return;
    item[campo] = campo === 'nome' ? valor : (parseFloat(valor) || 0);
    atualizarTotalEdicaoOrcamento();
}

function removerItemEdicaoOrcamento(indice) {
    itensEdicaoOrcamento.splice(indice, 1);
    if (!itensEdicaoOrcamento.length) adicionarItemEdicaoOrcamento();
    else renderizarItensEdicaoOrcamento();
}

function renderizarItensEdicaoOrcamento() {
    const corpo = document.getElementById('editar-orcamento-itens');
    if (!corpo) return;
    corpo.innerHTML = itensEdicaoOrcamento.map((item, indice) => {
        ...
        return `<tr>
            <td><input class="input-style" style="margin:0; min-width:220px;" value="${nome}" placeholder="Descrição do item ou serviço" oninput="atualizarItemEdicaoOrcamento(${indice}, 'nome', this.value)"></td>
            <td><input class="input-style" style="margin:0;" type="number" min="1" step="1" value="${item.qtd || 1}" oninput="atualizarItemEdicaoOrcamento(${indice}, 'qtd', this.value)"></td>
            <td><input class="input-style" style="margin:0;" type="number" min="0" step="0.01" value="${item.unitario || 0}" oninput="atualizarItemEdicaoOrcamento(${indice}, 'unitario', this.value)"></td>
            <td id="editar-orc-subtotal-${indice}" style="text-align:right; font-weight:700;">R$ ${subtotal.toFixed(2)}</td>
            <td style="text-align:center;"><button type="button" class="btn btn-sm btn-secondary" title="Remover item" aria-label="Remover item" onclick="removerItemEdicaoOrcamento(${indice})"><i class="ri-delete-bin-line"></i></button></td>
        </tr>`;
    }).join('');
    atualizarTotalEdicaoOrcamento();
}

function atualizarTotalEdicaoOrcamento() {
    const total = itensEdicaoOrcamento.reduce((soma, item) => soma + ((parseFloat(item.qtd) || 0) * (parseFloat(item.unitario) || 0)), 0);
    const totalEl = document.getElementById('editar-orc-valor');
    if (totalEl) totalEl.value = `R$ ${total.toFixed(2)}`;
    itensEdicaoOrcamento.forEach((item, indice) => {
        const subtotalEl = document.getElementById(`editar-orc-subtotal-${indice}`);
...
function abrirPopupEdicaoOrcamento(id) {
    ...
    itensEdicaoOrcamento = normalizarItensDocumento(orcamento).map(item => ({ ...item }));
    const resumo = document.getElementById('editar-orcamento-resumo');
    ...
    if (resumo) {
        resumo.innerHTML = `
            ...
            <div style="font-size:13px; color:var(--text-main); line-height:1.6;">Altere os dados e os itens abaixo antes de salvar.</div>
        `;
    }
...
async function salvarEdicaoOrcamentoDireta() {
    ...
    const itensValidos = itensEdicaoOrcamento.filter(item => String(item.nome || '').trim() && (parseInt(item.qtd) || 0) > 0);
    if (!itensValidos.length) return alert('O orçamento precisa ter pelo menos um item com descrição e quantidade.');
    const itens = itensValidos.map(item => {
        const qtd = parseInt(item.qtd) || 1;
        const unitario = parseFloat(item.unitario) || 0;
        return { ...item, nome: String(item.nome).trim(), qtd, unitario, total: qtd * unitario };
    });
    ...
    const payload = {
        ...
        peca: resumoItensVenda(itens),
        produtoId: itens.length === 1 ? (itens[0].produtoId || '') : '',
        qtd: itens.reduce((soma, item) => soma + item.qtd, 0),
        venda: total,
        unitario: itens.length === 1 ? itens[0].unitario : 0,
    ...
    const agendaRevisao = calcularAgendaRevisaoVenda(itens, payload);
    const proximaRevisao = agendaRevisao.slice().sort((a, b) => a.proxima_revisao_ts - b.proxima_revisao_ts)[0] || null;
    ...
    if (typeof registrarAuditoria === 'function') registrarAuditoria('VENDAS', orcamentoPopupEdicaoId, 'ORCAMENTO_EDITADO_DIRETO', { cliente: payload.cliente, pagamento: payload.pagamento, itens: itens.length, valor: total });
